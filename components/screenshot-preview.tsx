...
import { ScreenshotErrorBoundary } from '@/components/error-boundary';
import { apiRequest, FrontendError } from '@/lib/error-handling';
...
export function ScreenshotPreview({ targetId, targetName }: ScreenshotPreviewProps) {
  ...
  const fetchScreenshot = useCallback(async () => {
    try {
      ...
      const data = await apiRequest<{ screenshots: ScreenshotData[] }>(`/api/targets/${targetId}/screenshot`);
      ...
      if (error instanceof FrontendError) {
        ...
        setFetchError('Failed to load screenshots');
      }
    } finally {
      setLoading(false);
    }
              ...
              {imageError ? (
                ...
                <div className="grid gap-4">
                  {screenshots.map((screenshot, index) => (
                    <ScreenshotErrorBoundary key={index}>
